...
import { PaymentGateway_void } from "@znode/clients/payment";
    ...
    try {
        ...
        if (userData?.userId && requestObject.orderType === ORDER_DATA_TYPE.APPROVAL_ROUTING && requestObject.orderNumber && requestObject.configurationSetCode) {
            ...
            if (orderDetails?.PaymentDetails?.PaymentStatusCode?.toLowerCase() === PAYMENT_STATUS.AUTHORIZED.toLowerCase() && orderDetails?.PaymentDetails?.PaymentTransactionToken) {
                const response = await PaymentGateway_void(requestObject.configurationSetCode as string, { transactionId: orderDetails.PaymentDetails?.PaymentTransactionToken });
                const voidResponse = convertCamelCase(response);
                if (voidResponse && voidResponse?.isSuccess) {
                    const payment = {
                        ...
                        paymentDetails: {
                            ...
                            PaymentTransactionToken: voidResponse.PaymentTransactionToken,
                            PaymentStatusCode: PAYMENT_STATUS.VOIDED,
                            PurchaseOrderNumber: orderDetails.PaymentDetails?.PurchaseOrderNumber,
                            ExternalTransactionId: voidResponse.ExternalTransactionId
                        },
                    ...
                    return sendError("Failed to void payment.", 403);
                }
            }
            else {
                return sendError("Invalid request data.", 400);
            }
        } else {
        ...
        return sendError("An error occurred while voiding payment" + String(error), 500);
    }
}