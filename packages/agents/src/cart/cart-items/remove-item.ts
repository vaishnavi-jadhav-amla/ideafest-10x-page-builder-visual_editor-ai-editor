import { AREA, errorStack, logServer } from "@znode/logger/server";

import { CommerceCollections_deleteItemByClassType } from "@znode/clients/cp";
import { ORDER_DATA_TYPE } from "@znode/constants/order";
import { convertCamelCase } from "@znode/utils/server";

/**
 * Removes a specified item from the cart.
 *
 * @param cartNumber - The unique identifier of the cart from which the item should be removed.
 * @param cartItemId - The unique identifier of the cart item to be removed.
 * @returns A promise that resolves to a boolean indicating the success of the operation.
 *          Returns true if the item was successfully removed, false otherwise.
 */
export async function removeItemFromCart(cartNumber: string, cartItemId: string): Promise<boolean> {
  try {
    const response = await CommerceCollections_deleteItemByClassType(ORDER_DATA_TYPE.CARTS, cartNumber, cartItemId);

    const formattedResponse = convertCamelCase(response);

    return formattedResponse.isSuccess || false;
  } catch (error) {
    logServer.error(AREA.CART, errorStack(error));
    return false;
  }
}
