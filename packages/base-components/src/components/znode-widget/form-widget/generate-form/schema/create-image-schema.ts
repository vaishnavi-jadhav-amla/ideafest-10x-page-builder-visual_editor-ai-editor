...
export function createImageSchema(attribute: IFormAttribute) {
  ...
  if (attribute?.validationList && Array.isArray(attribute.validationList)) {
    ...
    schema = schema.test("Image Validation", `${attribute.attributeName} validation failed`, function (value) {
